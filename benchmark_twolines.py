...
def work(start, end, DEFINITIONS, RULES, object_defs, filtered_defs, data, imsize=300):
  ...
  for img_id in tqdm(range(start, end)):
    line_class = random.randrange(0, 3)
    while True:
      try:
        vars = generate_vars(0, 3, alphabet=True)
        # vars = ['d', 'e', 'f']
        if line_class == 0:
            clauses = [f"{vars[0]} {vars[1]} = segment", f"{vars[2]} = s_angle {vars[0]} {vars[1]} {vars[2]} {90}"]
        elif line_class == 1:
            clauses = [f"{vars[0]} {vars[1]} = segment", f"{vars[2]} = midpoint {vars[2]} {vars[0]} {vars[1]}"]
        else:
            angle = random.sample(list(range(5, 85)), k=1)[0] # + list(range(110, 160)), k=1)[0]
