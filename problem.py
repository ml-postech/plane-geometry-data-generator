...
class Clause:
  ...
  def translate(self, mapping: dict[str, str], shuffle=False) -> Clause:
    points0 = []
    # shuffle = False
    for p in self.points:
      pcount = len(mapping) + 1
      if len(p) == 1 and ord(p) >= 97 and ord(p) < 123:
        name = p
      elif shuffle:
        while True:
          name = random.choice([chr(i) for i in range(97, 97 + 26)])
...
class Problem:
  ...
  @classmethod
  def from_txt(cls, data: str, translate: bool = True, shuffle: bool = False) -> Problem:
    ...
    if translate:
      return problem.translate(shuffle)
    return problem
  ...
  def translate(self, shuffle=False) -> Problem:  # to single-char point names
    """Translate point names into alphabetical."""
    mapping = {}
    clauses = []

    for clause in self.clauses:
      clauses.append(clause.translate(mapping, shuffle))
