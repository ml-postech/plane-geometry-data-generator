...
def work(start, end, DEFINITIONS, RULES, object_defs, filtered_defs, data, imsize=300):
  ...
  for img_id in tqdm(range(start, end)):
    object_def_idx = random.sample([0, 1, 2], k=1)[0]
    object_def = DEFINITIONS[random.sample(object_defs[object_def_idx], k=1)[0]]
    while True:
      try:
        n_vars = 4  # len(object_def.construction.args)
        vars = generate_vars(0, n_vars, alphabet=True)
        # vars = ['s', 'q', 'u', 'a']
        random.shuffle(vars)
        if object_def.construction.name == 'parallelogram':
          clauses = [f"{vars[0]} {vars[1]} {vars[2]} = triangle", f"{vars[3]} = parallelogram {vars[0]} {vars[1]} {vars[2]}"]
        else:
          clauses = [" ".join(vars) + " = " + object_def.construction.name]

        for _ in range(2):
          n_vars = random.randrange(2) + 1
          n_cons = 1 # if n_vars > 1 else random.randrange(1) + 1
        ...
        # if answer == 2:
        #   p_args = ['b', 'c', 'c', 'd']
        #   p_args = list(map(lambda x: g.get(x, lambda: int(x)), p_args))
        #   new_deps = g.add_piece('perp', p_args, deps)
        #   deps += new_deps
