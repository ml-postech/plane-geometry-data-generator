...
class Graph:
  ...
  def check(self, name: str, args: list[Point]) -> bool:
    ...
    if name == 'ncyclic':
      return self.check_ncyclic(args)
    if name == 'circle':
  ...
  def check_ncyclic(self, points: list[Point]) -> bool:
    if self.check_cyclic(points):
      return False
    return not nm.check_cyclic([p.num for p in points])
